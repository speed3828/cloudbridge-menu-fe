...
Write-Host "🔐 Security"
# 자체 구현한 보안 검사 스크립트 실행
python $PSScriptRoot/security_check.py
if ($LASTEXITCODE -eq 0) {
    Write-Host "Security checks passed (자체 구현 보안 점검 스크립트 사용)"
} else {
    Write-Host "Security checks failed (코드 검토 필요)" -ForegroundColor Red
}
...
Write-Host "   - 보안 검사: 자체 구현 보안 점검 스크립트 사용" 